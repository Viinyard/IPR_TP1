...
public class ServeurTCP {
	...
	public static synchronized void ajouterPrinterSocketActives(PrintWriter printer) {
		ServeurTCP.printerSocketActives.add(printer);
	}
	...
	public static synchronized void envoyerATouteLesSocketsActive(String message) throws IOException {
		for(PrintWriter p : ServeurTCP.printerSocketActives) {
			p.println(message);
			p.flush();
		}
	}
	...
	public static void traiterSocketCliente(Socket socketVersUnClient, String charset) {
		...
		try {
			...
			while ((s = ServeurTCP.recevoirMessage(reader)) != null) {
				...
				ServeurTCP.envoyerATouteLesSocketsActive(name + ">" + s);
			}

			ServeurTCP.enleverPrinterSocketActives(printer);
